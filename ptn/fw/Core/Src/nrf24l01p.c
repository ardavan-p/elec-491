...
HAL_StatusTypeDef tx_cmd(TxPayload* payload, uint8_t txsz)
{
  ...
  status = HAL_SPI_Transmit(&hspi2, txbuffer, txsz, SPI_TIMEOUT_MS);
...
HAL_StatusTypeDef tx_rx_cmd(TxPayload* payload, uint8_t txsz, uint8_t* rxbuffer, uint8_t rxsz)
{
  HAL_StatusTypeDef status = HAL_ERROR;
  uint8_t txbuffer[TX_BUF_SZ_BYTES] = {0};
  ...
  HAL_GPIO_WritePin(CHIP_SELECT_GPIO_Port, CHIP_SELECT_Pin, GPIO_PIN_RESET);
  status |= HAL_SPI_Transmit(&hspi2, txbuffer, txsz, SPI_TIMEOUT_MS);
  status |= HAL_SPI_Receive(&hspi2, rxbuffer, rxsz, SPI_TIMEOUT_MS);
  HAL_GPIO_WritePin(CHIP_SELECT_GPIO_Port, CHIP_SELECT_Pin, GPIO_PIN_SET);
